...
# Check if FT_CONTRACT_ID is set
if [ -z "$FT_CONTRACT_ID" ]; then
  echo "Error: Environment variable FT_CONTRACT_ID is not set."
  exit 1
fi
...
near contract call-function as-transaction "$FT_CONTRACT_ID" clean base64-args $(echo $KEYS | base64 -w 0) prepaid-gas '100.0 Tgas' attached-deposit '0 NEAR' sign-as "$FT_CONTRACT_ID" network-config "$1" sign-with-legacy-keychain send
